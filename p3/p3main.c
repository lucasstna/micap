        ...
        switch (SBUF0) {
	        ...
	        case 'd':
	            if (PCA0CPH0 < 186)
					...
					printf_fast_f("RPM mínimo atingido\n");
	        ...
	        case 'r':
				rpm = 30/(le_pulso()*10);
				if (rpm <= 6)
					printf_fast_f("Rotacao do motor <= 6 rpm\n");
				else
		            printf_fast_f("Rotacao do motor: %3.1f rpm\n", rpm);
				break;
...
void le_LM35() {
	...
	printf_fast_f("Temperatura do motor: %2.1f C\n", (ladc * 0.00059326171875 / 1) * 100);
}
...
int int_tc1() interrupt 3 { // occurs every 2.62144 ms
	TF1 = 0;
	counter += 0.00262144;
	if (counter > 3) IE1 = 1; // stop reading if pulse width > 3s
}
